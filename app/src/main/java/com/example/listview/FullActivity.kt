...
import kotlinx.android.synthetic.main.activity_full.view.*
...
class FullActivity(private val dayList: Array<Day>) : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        val parityColor = arrayOf("#438EFF", "#FD7C86", "#FCEB77")
        ...
        for (day in dayList) {
            ...
            if (lessList.isNotEmpty()) {
                ...
                for (lesson in lessList) {
                    ...
                    columnView.day.setCardBackgroundColor(Color.parseColor(parityColor[lesson.even_parity]))
                    columnView.lessonNumber.setTextColor(Color.parseColor(parityTextColor[lesson.even_parity]))
                    columnView.lessonType.setTextColor(Color.parseColor(parityTextColor[lesson.even_parity]))
                    columnView.subgroupNumber.setTextColor(Color.parseColor(parityTextColor[lesson.even_parity]))
                    columnView.lessonName.setTextColor(Color.parseColor(parityTextColor[lesson.even_parity]))
                    columnView.prepodName.setTextColor(Color.parseColor(parityTextColor[lesson.even_parity]))
                    columnView.audit.setTextColor(Color.parseColor(parityTextColor[lesson.even_parity]))
